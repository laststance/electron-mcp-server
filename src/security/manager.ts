...
export interface SecureExecutionContext {
  ...
  userAgent?: string;
  /**
   * Coarse classification of the request, used by the security pipeline to
   * decide whether sandboxing applies and how to label audit log entries.
   *
   * - `command`: invoke a named UI command (click/fill/hover) — args are JSON,
   *   not raw JS. The actual JavaScript is generated by command modules.
   * - `query`: read-only DOM/state query (`get_title`, `get_url`, `find_elements`).
   *   Low risk; bypasses sandbox.
   * - `eval`: arbitrary JavaScript supplied by caller (`electron_eval`).
   *   High risk; subject to `validateEvalContent` and security profile gates.
   * - `screenshot` / `logs` / `window_info`: I/O against the running app.
   */
  operationType: 'command' | 'query' | 'eval' | 'screenshot' | 'logs' | 'window_info';
}
